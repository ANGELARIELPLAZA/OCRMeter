...
<template>
  <aside :class="['sidebar border-end p-3 d-flex flex-column', isCollapsed ? 'collapsed' : '']">
    ...
    <ul class="nav flex-column">
      ...
      <!-- Información -->
      <li v-if="canSeeReportes || isAdmin" class="nav-section-title mt-3 d-flex justify-content-between align-items-center"
          @click="showInfo = !showInfo" style="cursor: pointer;">
        <span>
      ...
      <ul v-if="showInfo && (canSeeReportes || isAdmin)" class="nav flex-column ms-3">
        <li v-if="isAdmin"><router-link to="/info/api" class="nav-link"><i class="bi bi-code-slash me-2"></i><span v-if="!isCollapsed">API</span></router-link></li>
        <li v-if="canSeeReportes"><router-link to="/info/reportes" class="nav-link"><i class="bi bi-file-earmark-bar-graph me-2"></i><span v-if="!isCollapsed">Reporte</span></router-link></li>
      </ul>
